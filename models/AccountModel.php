	...
	trait AccountModel{
		public function modelRegister(){
			...
			if($check->rowCount() == 0){
				...
				$query = $conn->prepare("insert into customers_project set name=:var_name,email=:var_email,address=:var_address,phone=:var_phone,password=:var_password");
		...
		public function modelLogin(){
			...
			$password = md5($password);
			$conn = Connection::getInstance();
			$query = $conn->prepare("select email,id from customers_project where email=:var_email and password=:var_password");
