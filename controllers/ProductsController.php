<?php 
	...
	class ProductsController extends Controller{
		...
		public function smart(){
			$id = isset($_GET["id"])&&is_numeric($_GET["id"])?$_GET["id"]:0;
			$record = $this->modelGetRecord($id);
			$this->loadView("SmartView.php",["record"=>$record]);
		}
		//danh gia so sao
		public function rating(){
			...
			$query = $conn->prepare("insert into rating_project set product_id=:var_product_id, star=:var_star");
		...
		public function getStar($id,$star){
			$conn = Connection::getInstance();
			$query = $conn->query("select id from rating_project where product_id=$id and star=$star");
		...
		public function ajax(){
			...
			$conn = Connection::getInstance();
			$query = $conn->query("select * from products_project where name like '%$key%'");
