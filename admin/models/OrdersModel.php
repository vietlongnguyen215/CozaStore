<?php 
	trait OrdersModel{
		...
		public function modelRead($recordPerPage){
			...
			$query = $conn->query("select * from orders_project order by id,status desc limit $from, $recordPerPage");
		...
		public function modelTotal(){
			//---
			$conn = Connection::getInstance();
			$query = $conn->query("select id from orders_project");
			//lay tong so ban ghi
		...
		public function modelGetOrders($id){
			//---
			$conn = Connection::getInstance();
			$query = $conn->query("select * from orders_project where id = $id");
			//tra ve mot ban ghi
			return $query->fetch();
			//---
		}
		...
		public function modelGetCustomers($id){
			//---
			$conn = Connection::getInstance();
			$query = $conn->query("select * from customers_project where id = $id");
			//tra ve mot ban ghi
			return $query->fetch();
			//---
		}
		...
		public function modelDelivery($id){
			//---
			$conn = Connection::getInstance();
			$conn->query("update orders_project set status = 1 where id = $id");
		}
		//lay danh sach cac san pham trong talbe orderdetails
		public function modelListOrderDetails($id){
			//---
			$conn = Connection::getInstance();
			$query = $conn->query("select * from orderdetails_project where order_id = $id");
			//tra ve mot ban ghi
			return $query->fetchAll();
			//---
		}
